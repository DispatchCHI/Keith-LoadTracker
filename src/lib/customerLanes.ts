...
export function commodityChipKey(raw: string): string {
  const label = tallyLabel(raw);
  if (label) return label;
  return normalizePlaceName(raw);
}

export function commoditiesSameChip(a: string, b: string): boolean {
  const ka = commodityChipKey(a);
  const kb = commodityChipKey(b);
  if (!ka || !kb) return false;
  return ka === kb;
}
...
export function upsertCustomerLane(
  ...
  const commodity = cleanPlaceName(input.commodity) || "Trash (MSW)";
  ...
  if (!prev) {
    prev = Object.values(store.lanes).find(
      (lane) =>
        ...
        commoditiesSameChip(lane.commodity, commodity) &&
...
export function currentLanesByCustomer(store: CustomerLaneStore, asOf: string): CustomerLane[] {
  ...
  for (const lane of Object.values(store.lanes)) {
    ...
    const key = `${normalizePlaceName(lane.customer)}|${normalizePlaceName(lane.destination)}|${commodityChipKey(lane.commodity)}`;
...
export function commoditiesForCustomer(
  ...
  for (const lane of Object.values(store.lanes)) {
    ...
    const key = commodityChipKey(lane.commodity);
    if (!key || seen.has(key)) continue;
    const label = commodityRankLabel(lane.commodity) || lane.commodity.trim();
    if (!label) continue;
    seen.add(key);
    out.push(label);
  }
...
export function destinationsForCustomer(
  ...
  for (const lane of currentLanesByCustomer(store, asOf)) {
    if (isLaneStub(lane)) continue;
    if (!placesMatch(lane.customer, customer)) continue;
    if (!commoditiesSameChip(lane.commodity, commodity)) continue;
    const dest = lane.destination.trim();
    const key = normalizePlaceName(dest);
    if (!key || seen.has(key)) continue;
    seen.add(key);
